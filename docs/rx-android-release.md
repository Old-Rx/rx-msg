...
Manual runs accept an optional `release_tag`. A value matching `vMAJOR.MINOR.PATCH-rx.NUMBER`, such as `v7.0.1-rx.1`, creates a draft prerelease and attaches the signed APK and checksum. Reusing the tag refreshes an existing draft after the new Actions artifact is safely uploaded; the workflow refuses to modify a published release. Leave the input empty to build only the Actions artifact. Draft releases must remain unpublished until installation, startup, profile creation, and messaging smoke tests pass on a physical ARM64 Android device.
